...
final class AuthService
{
    ...
    public function resolveActor(): ?array
    {
        ...
        if ($sessionActor['type'] === 'resident') {
            ...
            if (!$row || (int)$row['auth_version'] !== (int)$sessionActor['auth_version']
                || (int)$row['occupancy_id'] !== (int)($sessionActor['occupancy_id'] ?? 0)
                || (int)$row['room_id'] !== (int)($sessionActor['room_id'] ?? 0)
                || (string)$row['phone_norm'] !== (string)($sessionActor['phone'] ?? '')) {
                $this->app->session()->revokeLocal();
                return null;
            }
            return [
                ...
                'auth_method' => 'phone', 'assurance' => 'low', 'phone_verified' => false,
            ];
        }
        $this->app->session()->revokeLocal();
        return null;
    ...
    /** Phone-only access is an explicit low-assurance policy; it does not prove phone ownership. */
    public function residentLogin(Request $request, array $input): array
    {
        Validator::only($input, ['phone']);
        $rawPhone = is_string($input['phone'] ?? null) && strlen($input['phone']) <= 32 ? $input['phone'] : '';
        try { $phone = Validator::phone($rawPhone); }
        catch (HttpException) { $phone = ''; }
        $ip = $this->app->security()->clientIp($request);
        $this->app->limiter()->hit('resident-login-ip', $ip, 12, 900, 900);
        $this->app->limiter()->hit('resident-login-ip-daily', $ip, 100, 86400, 3600);
        $row = $phone !== '' ? $this->residentForPhone($this->app->database()->pdo(), $phone) : null;
        ...
        if (!$accountAllowed || !$row) {
            $this->app->audit()->write($request, null, 'auth.resident_failed', 'resident', null, [
                'principal_hash'=>hash_hmac('sha256', $phone !== '' ? $phone : trim($rawPhone), $this->app->config->appKey()),
                'account_rate_limited'=>!$accountAllowed, 'auth_method'=>'phone',
            ]);
            throw new HttpException(401, 'ไม่สามารถเข้าใช้งานด้วยเบอร์นี้ได้ กรุณาตรวจเบอร์ที่แจ้งไว้กับหอพักหรือติดต่อผู้ดูแล', 'INVALID_CREDENTIALS');
        }
        // Recheck under a shared lock: move-out, phone changes and room reassignment cannot
        // alter the matched identity while this audit event is being committed.
        $actor = $this->app->database()->transaction(function(PDO $pdo) use($row,$phone,$request): array {
            $fresh = $this->residentForPhone($pdo, $phone, true);
            foreach (['id','auth_version','occupancy_id','room_id'] as $key) {
                if (!$fresh || (int)$fresh[$key] !== (int)$row[$key]) {
                    throw new HttpException(401, 'ข้อมูลห้องเปลี่ยนระหว่างเข้าใช้งาน กรุณาลองใหม่', 'INVALID_CREDENTIALS');
                }
            }
            $actor = [
                'type'=>'resident', 'id'=>(int)$fresh['id'], 'full_name'=>$fresh['full_name'],
                'name'=>$fresh['full_name'], 'phone'=>$fresh['phone_norm'], 'email'=>$fresh['email'],
                'auth_version'=>(int)$fresh['auth_version'], 'occupancy_id'=>(int)$fresh['occupancy_id'],
                'room_id'=>(int)$fresh['room_id'], 'room_code'=>$fresh['room_code'], 'role'=>'resident',
                'auth_method'=>'phone', 'assurance'=>'low', 'phone_verified'=>false,
            ];
            $this->app->audit()->writeStrict($request, $actor, 'auth.resident_login', 'resident', $fresh['id'], [
                'auth_method'=>'phone', 'assurance'=>'low', 'phone_verified'=>false,
                'occupancy_id'=>(int)$fresh['occupancy_id'],
            ]);
            return $actor;
        });
        $this->app->session()->login($actor);
    ...
    /** A phone must match exactly one active resident/occupancy; never choose an arbitrary room. */
    private function residentForPhone(PDO $pdo, string $phone, bool $lock = false): ?array
    {
        $sql = "SELECT r.id,r.full_name,r.phone_norm,r.email,r.auth_version,
                    o.id AS occupancy_id,o.room_id,rm.room_code
                FROM residents r
                JOIN occupancies o ON o.resident_id=r.id AND o.status='active'
                JOIN rooms rm ON rm.id=o.room_id AND rm.deleted_at IS NULL
                WHERE r.phone_norm=? AND r.active=1
                ORDER BY o.id DESC LIMIT 2" . ($lock ? ' FOR SHARE' : '');
