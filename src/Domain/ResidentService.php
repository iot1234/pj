...
final class ResidentService
{
    ...
    public function list(): array
    {
        ...
        foreach($rows as &$row){$row['id']=(int)$row['id'];$row['occupancy_id']=(int)$row['occupancy_id'];$row['room_id']=$row['room_id']!==null?(int)$row['room_id']:null;$row['active']=(bool)$row['active'];}
        return $rows;
    }
    ...
    {
        $statement=$this->app->database()->pdo()->prepare("SELECT r.id,r.full_name,r.phone_norm AS phone,r.email,r.line_user_id,rm.id AS room_id,rm.room_code FROM residents r JOIN occupancies o ON o.resident_id=r.id AND o.status='active' JOIN rooms rm ON rm.id=o.room_id WHERE r.id=?");
        $statement->execute([$id]);$row=$statement->fetch();if(!$row)throw new HttpException(404,'Resident not found','RESIDENT_NOT_FOUND');$row['id']=(int)$row['id'];$row['room_id']=(int)$row['room_id'];return $row;
    }

    /** @return array<string,mixed> */
    public function updateProfile(int $id,array $input): array
    {
        Validator::only($input,['full_name','email','line_user_id']);
        $name=Validator::string($input['full_name']??null,'full_name',1,150);$email=Validator::nullableEmail($input['email']??null);
        $lineProvided=array_key_exists('line_user_id',$input);
        $lineUserId=$lineProvided?(trim((string)$input['line_user_id'])?:null):null;
        ...
        $lock=$pdo->prepare('SELECT line_user_id FROM residents WHERE id=? AND active=1 FOR UPDATE');
        $lock->execute([$id]);$current=$lock->fetch();
        if(!$current)throw new HttpException(404,'Resident not found','RESIDENT_NOT_FOUND');
        if(!$lineProvided)$lineUserId=$current['line_user_id'];
        try{
            $update=$pdo->prepare('UPDATE residents SET full_name=?,email=?,line_user_id=?,updated_at=UTC_TIMESTAMP() WHERE id=? AND active=1');
            $update->execute([$name,$email,$lineUserId,$id]);
    ...
    {
        Validator::only($input,['full_name','phone','email','line_user_id']);
        ...
        $lineUserId=array_key_exists('line_user_id',$input)
            ?(trim((string)$input['line_user_id'])?:null)
            :($current['line_user_id']!==null?(string)$current['line_user_id']:null);
        if($lineUserId!==null&&!preg_match('/^U[0-9A-Za-z_-]{20,80}$/',$lineUserId)){
            throw new HttpException(422,'LINE User ID ไม่ถูกต้อง','VALIDATION_ERROR',['field'=>'line_user_id']);
        }
        ...
        foreach([
            ...
            'line_user_id'=>[$current['line_user_id'],$lineUserId],
        ] as $field=>$values){
        ...
        try{
            $update=$pdo->prepare('UPDATE residents SET full_name=?,phone_norm=?,email=?,line_user_id=?,auth_version=auth_version+?,updated_at=UTC_TIMESTAMP() WHERE id=? AND active=1');
            $update->execute([$name,$phone,$email,$lineUserId,$phoneChanged?1:0,$id]);
        }catch(PDOException $error){
            if((int)($error->errorInfo[1]??0)===1062||(string)$error->getCode()==='23000'){
                throw new HttpException(409,'เบอร์โทรหรือ LINE User ID นี้ผูกกับผู้พักรายอื่นแล้ว','RESIDENT_IDENTITY_IN_USE');
            }
            throw $error;
        }
