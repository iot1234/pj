        ...
        <main class="admin-content" id="main-content" tabindex="-1">
            ...
            <section class="admin-view" data-admin-view="bills" aria-labelledby="bills-title" hidden>
                ...
                <form class="panel billing-builder" id="bill-builder-form">
                    <div class="security-note" id="billing-readiness-note"><strong>ตรวจสอบค่ารายเดือนก่อนออกบิลล์</strong><span>ระบบกำลังโหลดสถานะจากหน้า “ตั้งค่า”</span></div>
                    <div class="form-grid form-grid-four">
                        ...
                        <label class="field"><span>จำนวนเงินอื่น</span><input type="number" name="other_amount" min="0" step="0.01" value="0"></label>
                    </div>
                    <fieldset class="room-selector"><legend>เลือกห้องที่จะออกบิลล์</legend><label class="check-field"><input type="checkbox" id="select-all-bill-rooms"><span>เลือกทุกห้อง</span></label><div class="room-check-grid" id="bill-room-options"><span class="muted">เลือกรอบเดือนเพื่อโหลดห้อง</span></div></fieldset>
                    <p class="form-error" id="bill-builder-error" role="alert" hidden></p>
                    <div class="form-actions"><button class="button button-secondary" type="button" id="preview-bills-button">ตรวจยอดก่อน</button><button class="button button-primary" type="submit">ออกบิลล์ที่เลือก</button></div>
                </form>
                <div class="section-subheading"><h3>บิลล์ในรอบเดือน</h3><button class="button button-secondary button-small" type="button" id="line-bulk-button" disabled>เข้าคิว LINE ทั้งหมด</button></div>
                <div class="panel table-panel"><div class="table-scroll"><table><thead><tr><th>เลขที่บิลล์</th><th>ห้อง</th><th>ผู้พัก</th><th>ยอดรวม</th><th>กำหนดชำระ</th><th>สถานะ</th><th class="align-right">LINE</th></tr></thead><tbody id="bill-admin-rows"></tbody></table></div><div class="table-state" id="bill-admin-state" data-state="idle"><p>เลือกรอบเดือนเพื่อดูบิลล์</p></div></div>
            </section>
            ...
            <section class="admin-view" data-admin-view="payments" aria-labelledby="payments-title" hidden>
                ...
                <div class="panel table-panel"><div class="table-scroll"><table><thead><tr><th>อัปโหลดเมื่อ</th><th>บิลล์ / ห้อง</th><th>ผู้พัก</th><th>ยอดเงิน</th><th>ผลตรวจ</th><th class="align-right">จัดการ</th></tr></thead><tbody id="payment-rows"></tbody></table></div><div class="table-state" id="payment-state" data-state="loading"><span class="spinner" aria-hidden="true"></span><p>กำลังโหลดการชำระ…</p></div></div>
            </section>
                    ...
                    <form class="panel settings-card settings-integration-card" id="integration-settings-form" data-owner-only="<?= $canManageIntegrations ? 'true' : 'false' ?>">
                        ...
                        <div class="integration-form-grid">
                            <fieldset class="integration-fieldset">
                                ...
                                <label class="field"><span>เบอร์พร้อมเพย์ / เลขผู้เสียภาษี</span><input name="promptpay_target" type="text" inputmode="numeric" maxlength="13" pattern="(?:0[0-9]{9}|[0-9]{13})" placeholder="0812345678"<?= $integrationDisabled ?>><small>ใช้สร้าง QR ตามยอดบิลล์</small></label>
                            ...
                            <fieldset class="integration-fieldset integration-fieldset-wide">
                                ...
                                <div class="form-grid form-grid-two">
                                     <label class="field"><span>ผู้ให้บริการ</span><select name="slip_provider"<?= $integrationDisabled ?>><option value="none">ยังไม่เปิดใช้</option><option value="slipok">SlipOK</option><option value="easyslip">EasySlip</option></select></label>
                                    ...
                                    <label class="field"><span>SlipOK Branch ID</span><input name="slipok_branch_id" type="text" maxlength="80" pattern="[A-Za-z0-9_-]{1,80}" placeholder="จำเป็นเมื่อเลือก SlipOK"<?= $integrationDisabled ?>></label>
                                    <label class="field"><span>SlipOK API Key</span><input name="slipok_api_key" type="password" maxlength="4096" autocomplete="new-password" placeholder="เว้นว่างเพื่อเก็บค่าเดิม"<?= $integrationDisabled ?>><small data-secret-status="slipok_api_key">ยังไม่ได้โหลดสถานะ</small></label>
                                    <label class="field"><span>EasySlip API Key</span><input name="easyslip_api_key" type="password" maxlength="4096" autocomplete="new-password" placeholder="เว้นว่างเพื่อเก็บค่าเดิม"<?= $integrationDisabled ?>><small data-secret-status="easyslip_api_key">ยังไม่ได้โหลดสถานะ</small></label>
                                    <label class="check-field danger-check"><input name="slipok_api_key_clear" type="checkbox" value="1"<?= $integrationDisabled ?>><span>ล้าง SlipOK API Key</span></label>
                                    <label class="check-field danger-check"><input name="easyslip_api_key_clear" type="checkbox" value="1"<?= $integrationDisabled ?>><span>ล้าง EasySlip API Key</span></label>
...
<dialog class="modal" id="move-in-dialog" aria-labelledby="move-in-title">
    <form class="modal-card" id="move-in-form"><input type="hidden" name="booking_id"><div class="modal-header"><div><p class="eyebrow">เปิดบัญชีผู้พัก</p><h2 id="move-in-title">รับเข้าพัก</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="modal-lead" id="move-in-summary"></p><div class="form-grid"><label class="field"><span>PIN เริ่มต้น</span><input name="pin" type="password" inputmode="numeric" pattern="[0-9]{6,12}" minlength="6" maxlength="12" required autocomplete="new-password"><small>6–12 หลัก แจ้งให้ผู้พักเพื่อเข้าระบบ</small></label><label class="field"><span>อีเมล (ไม่บังคับ)</span><input name="email" type="email" maxlength="254" autocomplete="email"></label><label class="field"><span>LINE User ID (ไม่บังคับ)</span><input name="line_user_id" type="text" minlength="21" maxlength="81" pattern="U[0-9A-Za-z_-]{20,80}" autocomplete="off"></label><label class="field"><span>วันที่เข้าพัก</span><input name="move_in_date" type="date" required></label></div><label class="check-field" id="move-in-reuse-field" hidden><input name="reuse_resident_id" type="checkbox" disabled><span id="move-in-reuse-label">ยืนยันการเชื่อมบัญชีผู้พักเดิม</span></label><p class="field-hint" id="move-in-reuse-help" hidden>เลือกเฉพาะเมื่อยืนยันแล้วว่าเป็นบุคคลเดิม ประวัติบิลเก่าจะถูกเชื่อมกับบัญชีนี้ หากเป็นคนละคนต้องใช้เบอร์โทรอื่นเพื่อปกป้องข้อมูลส่วนบุคคล</p><p class="form-error" id="move-in-error" role="alert" hidden></p><div class="form-actions"><button class="button button-ghost" type="button" data-close-dialog>ยกเลิก</button><button class="button button-primary" type="submit">ยืนยันเข้าพัก</button></div></form>
</dialog>

<dialog class="modal" id="resident-pin-reset-dialog" aria-labelledby="resident-pin-reset-title">
    <form class="modal-card" id="resident-pin-reset-form"><input type="hidden" name="resident_id"><div class="modal-header"><div><p class="eyebrow">ความปลอดภัยผู้พัก</p><h2 id="resident-pin-reset-title">ตั้ง PIN ใหม่</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="modal-lead" id="resident-pin-reset-summary"></p><div class="security-note"><strong>เซสชันเดิมจะออกจากระบบทันที</strong><span>หลังบันทึก ผู้พักต้องใช้ PIN ใหม่ในการเข้าสู่ระบบ</span></div><div class="form-grid form-grid-two"><label class="field"><span>PIN ใหม่</span><input name="new_pin" type="password" inputmode="numeric" pattern="[0-9]{6,12}" minlength="6" maxlength="12" required autocomplete="new-password"><small>6–12 หลัก และไม่เป็นเลขซ้ำหรือลำดับที่เดาง่าย</small></label><label class="field"><span>ยืนยัน PIN ใหม่</span><input name="confirm_pin" type="password" inputmode="numeric" pattern="[0-9]{6,12}" minlength="6" maxlength="12" required autocomplete="new-password"></label></div><p class="form-error" id="resident-pin-reset-error" role="alert" hidden></p><div class="form-actions"><button class="button button-ghost" type="button" data-close-dialog>ยกเลิก</button><button class="button button-primary" type="submit">บันทึก PIN ใหม่</button></div></form>
</dialog>

<dialog class="modal" id="resident-edit-dialog" aria-labelledby="resident-edit-title">
    <form class="modal-card" id="resident-edit-form"><input type="hidden" name="resident_id"><div class="modal-header"><div><p class="eyebrow">ข้อมูลผู้พักที่ยืนยันแล้ว</p><h2 id="resident-edit-title">แก้ข้อมูลผู้พัก</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="modal-lead" id="resident-edit-summary"></p><div class="security-note"><strong>เบอร์โทรคือชื่อบัญชีเข้าสู่ระบบ</strong><span>เมื่อเปลี่ยนเบอร์ ระบบจะยกเลิกเซสชันเดิมทันที ผู้พักต้องเข้าสู่ระบบใหม่ด้วยเบอร์ใหม่และ PIN เดิม</span></div><div class="form-grid form-grid-two"><label class="field"><span>ชื่อ–นามสกุล</span><input name="full_name" type="text" minlength="1" maxlength="150" required autocomplete="name"></label><label class="field"><span>เบอร์โทรศัพท์</span><input name="phone" type="tel" minlength="10" maxlength="20" required autocomplete="tel"></label><label class="field"><span>อีเมล</span><input name="email" type="email" maxlength="190" autocomplete="email"></label><label class="field"><span>LINE User ID</span><input name="line_user_id" type="text" minlength="21" maxlength="81" pattern="U[0-9A-Za-z_-]{20,80}" autocomplete="off" placeholder="U ตามด้วยรหัสบัญชี LINE"></label></div><p class="form-error" id="resident-edit-error" role="alert" hidden></p><div class="form-actions"><button class="button button-ghost" type="button" data-close-dialog>ยกเลิก</button><button class="button button-primary" type="submit">บันทึกข้อมูล</button></div></form>
</dialog>

<dialog class="modal" id="resident-move-out-dialog" aria-labelledby="resident-move-out-title">
    <form class="modal-card" id="resident-move-out-form"><input type="hidden" name="resident_id"><div class="modal-header"><div><p class="eyebrow">สิ้นสุดการเข้าพัก</p><h2 id="resident-move-out-title">ย้ายผู้พักออก</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="modal-lead" id="resident-move-out-summary"></p><div class="security-note"><strong>ตรวจบิลล์ปิดรอบก่อนย้ายออก</strong><span>ต้องไม่มีบิลล์ค้าง และต้องมีบิลล์ของเดือนที่ย้ายออกซึ่งชำระแล้ว กรุณาจดมิเตอร์ปลายงวดและออกบิลล์ให้ครบก่อนดำเนินการ</span></div><div class="form-grid"><label class="field"><span>วันที่ย้ายออก</span><input name="move_out_date" type="date" required></label></div><p class="form-error" id="resident-move-out-error" role="alert" hidden></p><div class="form-actions"><button class="button button-ghost" type="button" data-close-dialog>ยกเลิก</button><button class="button button-danger" type="submit">ยืนยันย้ายออก</button></div></form>
</dialog>

<dialog class="modal" id="payment-close-dialog" aria-labelledby="payment-close-title">
    <form class="modal-card" id="payment-close-form"><input type="hidden" name="payment_id"><div class="modal-header"><div><p class="eyebrow">กู้รายการตรวจสลิปค้าง</p><h2 id="payment-close-title">ปิดรายการชำระนี้</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="modal-lead" id="payment-close-summary"></p><div class="security-note"><strong>การปิดรายการไม่ทำให้บิลล์เป็นชำระแล้ว</strong><span>หลักฐานนี้จะถูกปฏิเสธและเก็บไว้ตรวจสอบ ผู้พักจึงสามารถส่งสลิปใหม่ได้</span></div><div class="form-grid"><label class="field"><span>เหตุผล</span><textarea name="reason" minlength="3" maxlength="450" rows="4" required placeholder="เช่น ผู้ให้บริการไม่ตอบสนองหลังตรวจซ้ำแล้ว"></textarea></label></div><p class="form-error" id="payment-close-error" role="alert" hidden></p><div class="form-actions"><button class="button button-ghost" type="button" data-close-dialog>ยกเลิก</button><button class="button button-danger" type="submit">ปิดและให้ส่งสลิปใหม่</button></div></form>
</dialog>
...
<dialog class="modal" id="preview-dialog" aria-labelledby="preview-title">
    <div class="modal-card modal-card-wide"><div class="modal-header"><div><p class="eyebrow">ยังไม่สร้างบิลล์</p><h2 id="preview-title">ตรวจยอดก่อนออกบิลล์</h2></div><button class="icon-button" type="button" data-close-dialog aria-label="ปิด">×</button></div><div class="preview-list" id="bill-preview-content"></div><div class="form-actions"><button class="button button-primary" type="button" data-close-dialog>ตรวจแล้ว</button></div></div>
</dialog>
