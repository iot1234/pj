...
15. Deploy source ปัจจุบันโดยยังปิด public traffic แล้วให้ Owner เข้าผ่านช่องทาง maintenance เพื่อ reissue credential ผู้พักเดิมที่ยังเข้าไม่ได้และบันทึก integration settings รวม LINE Basic ID
16. ให้ production รัน `php scripts/check_requirements.php --db --strict --production` ด้วยบัญชี runtime แล้วรัน `php scripts/check_requirements.php --schema-audit` ด้วยบัญชี DBA/schema owner ชั่วคราว Canonical schema ต้องพบ 16 ตาราง, integrity triggers 19 รายการพร้อม body ตรง canonical และ CHECK 86 รายการ รวม booking/occupancy insert guards, claim/lease, heartbeat, resident credential, occupancy-meter, LINE binding, LINE Basic ID และ move-in request hash guards
17. เริ่ม worker หลัง migration/deploy สำเร็จและตรวจว่า `notification_worker_heartbeats.heartbeat_at` เดินต่อเนื่อง ห้ามถือว่า web `/healthz.php` ที่ผ่านเพียงอย่างเดียวพิสูจน์ว่า worker ส่ง LINE ได้ เพราะ web readiness ตั้งใจไม่ผูกกับ liveness ของ service อื่น
