...
| FR-02 | Guest จองด้วยชื่อ/เบอร์ แล้วห้องเป็น “จองแล้ว” รอ Admin | `POST /api/public/bookings` | `bookings`; normalized Thai phone, idempotency key, generated unique active room, IP/phone rate limit | request ซ้ำด้วย key เดิมไม่สร้างซ้ำ; booking พร้อมกันห้องเดียวสำเร็จได้หนึ่งรายการ; invalid/unavailable room ไม่กิน phone quota; status เป็น `pending`, ห้อง derived เป็น `reserved` |
| FR-03 | Resident login ด้วยเบอร์ที่ผูกห้อง พร้อม PIN | `/resident/login`, `POST /api/auth/resident/login`, logout/me | `residents`, active `occupancies`; PIN hash, IP/account rate limit, generic error, session rotation | เบอร์+PIN ถูกและมี occupancy active เข้าได้; inactive/ไม่มีห้อง/PIN ผิดเข้าไม่ได้และไม่บอกว่าข้อมูลใดผิด; account ที่ block แล้วใช้ PIN ถูกข้าม limit ไม่ได้ |
...
| FR-05 | Resident ดูและแก้ข้อมูลส่วนตัว | `GET|PUT /api/resident/profile`, `POST /api/resident/profile/pin`; Admin ใช้ `PUT /api/admin/residents/{id}` เมื่อยืนยันตัวตนแล้ว | `residents`; allowlist ชื่อ, email, LINE User ID และ PIN; เบอร์เป็น login identity | Resident แก้ชื่อ/email/LINE User ID ได้และเปลี่ยน PIN ต้องยืนยัน PIN เดิม; แก้ room/occupancy/role/phone ผ่าน resident payload ไม่ได้; Admin เปลี่ยนเบอร์แล้ว revoke session เดิม |
...
| FR-14 | ส่งบิล LINE รายห้อง/พร้อมกัน | `POST /api/admin/bills/{id}/line`, `POST /api/admin/bills/line-bulk` | `notification_outbox`, `integration_settings`; token เข้ารหัส, unique bill/purpose, stable UUID v4 retry key, backoff | ไม่มี LINE ID ถูก skip/แจ้งชัด; retry ใช้ `X-Line-Retry-Key` เดิม; 409 ถือว่าได้รับแล้ว; terminal failure มองเห็นได้ |
| FR-15 | สร้าง QR พร้อมเพย์ตามยอดบิล | `GET /api/resident/bills/{id}/promptpay` | `bills.total_amount` snapshot + PromptPay target ใน `integration_settings`; EMV CRC | QR amount มาจากบิลของ resident ที่ login เท่านั้น; target/amount invalid ถูกปฏิเสธ; CRC ถูกต้อง |
