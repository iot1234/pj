...
transfer_instructions.active_amount|decimal(14,2)|YES|STORED GENERATED|casewhenstatusin'reserved','settled'thentransfer_amountelsenullend"
...
verify_schema_and_defaults() {
    ...
    [[ "$object_count" == 22 && "$base_table_count" == 22 ]] \
        || die "database is nonempty but does not contain exactly the required 22 base tables; refusing to modify it"
    ...
    [[ "$actual_trigger_count" == 26 ]] \
        || die "database does not contain exactly the required 26 integrity triggers; refusing to modify it"
    ...
    (( check_constraint_count >= 119 )) \
        || die "database has fewer than the required 119 CHECK constraints"
    ...
    DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_DATABASE="$DB_DATABASE" DB_USERNAME="$DB_USERNAME" DB_PASSWORD="$DB_PASSWORD" \
        ...
        || die "database guard schema does not match migrations 014/015"
