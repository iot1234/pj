...
verify_schema_and_defaults() {
    ...
    [[ "$object_count" == 16 && "$base_table_count" == 16 ]] \
        || die "database is nonempty but does not contain exactly the required 16 base tables; refusing to modify it"
    ...
    [[ "$actual_trigger_count" == 19 ]] \
        || die "database does not contain exactly the required 19 integrity triggers; refusing to modify it"
    ...
    (( check_constraint_count >= 86 )) \
        || die "database has fewer than the required 86 CHECK constraints"
