...
require dirname(__DIR__).'/src/Support/PendingOpeningSchema.php';
try{
    ...
    $errors=array_merge(Dormitory\Support\LinePlatformSchema::errors($pdo),Dormitory\Support\PendingOpeningSchema::errors($pdo));
    foreach($errors as$error)fwrite(STDERR,$error."\n");
    if($errors!==[])exit(1);
    fwrite(STDOUT,"LINE platform schema, legacy metadata and migration 015 opening-reading guard verified\n");
}catch(Throwable){fwrite(STDERR,"Could not verify database guards; check migrations 014/015 and the database connection\n");exit(1);}
