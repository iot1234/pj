...
ผู้พักที่ login อยู่เริ่มผูก LINE จากหน้าโปรไฟล์ได้โดยไม่ต้องกรอก PIN หน้าเดียวกันมีปุ่มเพิ่มเพื่อนจาก LINE Basic ID ที่ระบบตรวจสอบและดึงจาก LINE ระบบสร้างรหัส `BIND-` จากข้อมูลสุ่ม 128 บิต แสดงเพียงครั้งเดียวพร้อม QR/countdown และให้ส่งรหัสนี้ในแชตส่วนตัวกับ LINE Official Account ภายใน 10 นาที หน้าจอตรวจสถานะให้อัตโนมัติ ฐานข้อมูลเก็บเฉพาะ HMAC-SHA256 ของรหัส ไม่เก็บ bearer code แบบอ่านกลับได้ Webhook ที่ลายเซ็นถูกต้องจะผูก LINE User ID จาก event กับ resident ภายใต้ transaction/row lock แล้วบันทึก audit การยืนยัน รหัสหมดอายุ ใช้แล้ว ถูกเพิกถอน หรือใช้กับผู้พักที่ไม่มี occupancy active จะถูกปฏิเสธ ค่า LINE เดิมที่ไม่มี audit การยืนยันจะขึ้นว่า “รอยืนยันใหม่” และ worker จะไม่ส่งจนกว่าจะผ่านขั้นตอนนี้
...
ปุ่มส่งบิล LINE รายห้อง/ทั้งงวดแนบ **Flex Message พร้อม QR, ยอดบิล, ยอดโอนที่ล็อก, ห้อง, รอบบิล และปุ่มเปิดเว็บส่งสลิป** แล้ว ผู้พักที่ผูกกับบอทหลักพิมพ์ `บิล` เพื่อขอ QR ของบิลล่าสุดได้ด้วย ใช้ยอดจาก `transfer_instructions` เดียวกับหน้าเว็บ: เพิ่ม 0.01–0.99 บาท ไม่สุ่มใหม่ตอนส่ง LINE และไม่เปลี่ยนยอดต้นฉบับในใบแจ้งหนี้ ไม่ต้องเปิด SlipOK/EasySlip เพื่อใช้ QR

สิ่งที่ต้องพร้อม:

1. `APP_URL` เป็น HTTPS สาธารณะที่ LINE เข้าถึงได้จริงและใบรับรองถูกต้อง พร้อม PHP GD (Docker image มีอยู่แล้ว)
2. Owner ตั้งผู้รับ PromptPay จริงที่หน้า ตั้งค่า และเชื่อม LINE OA หลักตามขั้นตอนเดิม ผู้พักต้องผูก LINE กับห้องก่อน
3. เปิด notification worker สำหรับการส่งจากหลังบ้าน; คำสั่ง `บิล` ตอบผ่าน webhook เดิม
4. Reverse proxy/WAF ต้องยอมให้ GET `/api/public/line-payment-qr/<id>?token=...` ผ่านได้โดยไม่บังคับ login/cookie **แต่ห้ามเอาการตรวจ token ออกจากแอป** ตั้ง proxy access log ให้ปิดบัง query ของเส้นทางนี้ด้วย

รูปสร้างเป็น PNG บนเซิร์ฟเวอร์ ไม่ส่งเลขบัญชี/ยอดให้เว็บสร้าง QR ภายนอก ลิงก์รูปมี HMAC และอายุไม่เกิน 30 วัน ผูกกับบิล ยอด ผู้พัก รอบเข้าพัก auth version และผู้รับ LINE คนเดิม ไม่เปิดสิทธิ์อ่านรายละเอียดบิลโดยไม่ login การเปิดรูปไม่จองยอดใหม่ เมื่อมีสลิปรอตรวจ/ผ่านตรวจ/บิลชำระแล้ว, เปลี่ยนผู้รับเงิน, เปลี่ยนเบอร์, ยกเลิกผูก หรือปิดบอท จะไม่คืนรูป QR ใหม่ให้ลิงก์เดิม Worker ตรวจซ้ำก่อน push และ retry ใช้ข้อความ/ผู้รับ/URL/retry key เดิม

กรณี HTTP-only, ไม่มี PromptPay หรือจองครบ 99 ยอดในช่วงเดียวกัน ระบบส่งข้อความพร้อมลิงก์ให้ตรวจบิลแทนรูป ไม่สร้าง QR ยอดผิดหรือเปลี่ยนบัญชีเอง หากลิงก์หมดอายุให้พิมพ์ `บิล` ใหม่ ยอดเดิมยังถูกล็อกไว้ รายการส่งสำเร็จแล้วไม่ถูกส่งซ้ำอัตโนมัติ

**รูปที่ LINE cache หรือผู้ใช้บันทึกไว้แล้วไม่สามารถเรียกคืนได้** ต้องอ่านข้อความเตือนและตรวจสถานะล่าสุดก่อนโอน หากโอนแล้วไม่ต้องโอนซ้ำ การส่งสลิปในแชตยังเป็นการให้ผู้ดูแลตรวจ ไม่ได้อัปโหลดเข้าเว็บหรือยืนยัน `paid` อัตโนมัติ

รายละเอียดโครงสร้างและผลทดสอบ: [LINE_PAYMENT_QR.md](docs/LINE_PAYMENT_QR.md)
