...
- LINE: กรอก Channel access token, Channel secret, จำนวน retry 1–20 ครั้ง และ batch size 1–100 งาน จากนั้นคัดลอก Webhook URL ที่หน้า Settings แสดง (`<APP_URL>/api/webhooks/line`) ไปใส่ใน LINE Developers Console เปิด **Use webhook** และ **Webhook redelivery** แล้วกด **Verify** ต้องตั้ง `APP_URL` เป็น HTTPS origin สาธารณะที่ตรงกับโดเมนจริง Bot รับ event `follow` และข้อความตัวอักษรจากแชตผู้ใช้โดยตรง แต่ไม่ตอบ LINE User ID ดิบและไม่เก็บเนื้อหาข้อความ ผู้พักสร้างรหัส `BIND-` จากหน้าโปรไฟล์แล้วส่งรหัสนั้นให้ Bot ภายใน 10 นาทีเพื่อผูก `source.userId` ส่วนการส่งบิลใช้ `https://api.line.me/v2/bot/message/push`, retry ด้วย `X-Line-Retry-Key` เดิม และถือ HTTP 409 ว่าคำขอเดิมได้รับแล้วเฉพาะเมื่อ LINE ส่ง `x-line-accepted-request-id` ที่ถูกต้องกลับมา มิฉะนั้นจะ retry/fail ตามนโยบายแทนการถือว่าสำเร็จ
...
`--production` บังคับ `APP_ENV=production`, เปิดการตรวจฐานข้อมูล และถือ warning เป็น failure ในคำสั่งเดียว เหมาะเป็น deployment gate ของ production ส่วน `--schema-audit` ใช้ชั่วคราวกับบัญชี DBA/schema owner เพื่อตรวจ trigger ทั้ง 19 รายการรวม body ไม่ใช่คำสั่งสำหรับบัญชี runtime ประจำของแอป Fresh schema หลัง migration `012` ต้องมี 16 ตารางและ CHECK 86 รายการ โดยตัวตรวจจะตรวจทั้งจำนวนขั้นต่ำและ named guards ของ claim lease, heartbeat, resident credential, occupancy-meter, LINE binding code, LINE Basic ID และ move-in request hash
