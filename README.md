...
> **คำเตือนด้านการยืนยันตัวตนผู้พัก:** หน้า Resident ใช้เฉพาะเบอร์โทรที่ผูกกับผู้พักและห้องที่ยัง active ไม่มี PIN, password, OTP login หรือ trusted-device bypass ผู้ที่รู้เบอร์ดังกล่าวจึงสามารถสวมบัญชีผู้พักได้ทันที rate limit ช่วยลดการยิงจำนวนมากแต่ป้องกันการสวมบัญชีครั้งแรกไม่ได้ รูปแบบนี้เป็นการยืนยันตัวตนระดับต่ำและไม่เหมาะกับข้อมูล/ธุรกรรมที่ต้องการความมั่นใจสูง บัญชี Admin/Owner ยังคงใช้ username และ password ตามเดิม

Resident session หมดอายุเมื่อไม่มีการใช้งาน 15 นาทีหรือครบอายุรวม 1 ชั่วโมง แล้วแต่กรณีใดถึงก่อน ผู้พักยังแก้ชื่อและ email ของตนได้ แต่แก้เบอร์/ห้องไม่ได้ ไม่มี route เปลี่ยนหรือรีเซ็ต PIN และการย้ายเข้าไม่รับ PIN
...
ฐานข้อมูลเดิมต้องสำรองและทดสอบ restore ก่อน แล้วใช้บัญชี schema/migration ที่มีสิทธิ์ DDL (ไม่ใช่ `DB_USERNAME` ของแอป) รันตามลำดับ: `database/migrations/001_integration_settings.sql` เมื่อฐานยังไม่มี `integration_settings` (ไฟล์นี้ idempotent), รัน `database/migrations/002_operational_hardening.sql` **หนึ่งครั้งเท่านั้น**, รัน `database/migrations/003_append_only_guards.sql`, `database/migrations/004_line_webhook.sql` และ `database/migrations/005_booking_active_phone.sql` ตามเงื่อนไขเดิม จากนั้น deploy transitional commit `a52bc33` ให้ทุก replica healthy ก่อนรัน `database/migrations/006_remove_resident_pin.sql` เพื่อลบ `residents.pin_hash` ตรวจว่าคอลัมน์หายแล้วจึง deploy source ปัจจุบัน ห้ามรัน `006` ขณะยังมีแอปรุ่น PIN เพราะแอปรุ่นนั้นยังอ้างคอลัมน์นี้ และหลังลบแล้วจะ rollback ไปแอปรุ่น PIN ไม่ได้โดยไม่ restore schema/backup

Fresh schema/`install.sql` ไม่มี `pin_hash` อยู่แล้วและไม่ต้องรัน migration ส่วน source ปัจจุบันไม่มีโค้ด compatibility ที่อ่านหรือเขียนคอลัมน์นี้ ฐานเดิมจึงต้องผ่านลำดับ `001` (เมื่อจำเป็น) → `002` หนึ่งครั้ง → `003` → `004` → `005` → transitional commit `a52bc33` → `006` → ตรวจ schema → deploy source ปัจจุบัน จากนั้น login ด้วยบัญชี `owner` แล้วกรอกค่าที่ Admin → ตั้งค่า → “พร้อมเพย์, LINE Bot และตรวจสลิป” ระบบไม่ย้ายหรืออ่านค่าดำเนินงานเดิมจาก environment โดยอัตโนมัติ จึงต้องกรอกใหม่ในหน้าหลังบ้านก่อนเปิด LINE webhook/ส่งบิล/ตรวจสลิป/PromptPay แล้วรัน `php scripts/check_requirements.php --db --strict` ด้วยบัญชี runtime และ `php scripts/check_requirements.php --schema-audit` ด้วยบัญชี migration ชั่วคราวอีกครั้ง
...
- LINE: กรอก Channel access token, Channel secret, จำนวน retry 1–20 ครั้ง และ batch size 1–100 งาน จากนั้นคัดลอก Webhook URL ที่หน้า Settings แสดง (`<APP_URL>/api/webhooks/line`) ไปใส่ใน LINE Developers Console เปิด **Use webhook** และ **Webhook redelivery** แล้วกด **Verify** ต้องตั้ง `APP_URL` เป็น HTTPS origin สาธารณะที่ตรงกับโดเมนจริง Bot จะตอบ event `follow` และข้อความตัวอักษรจากแชตผู้ใช้โดยตรงด้วย LINE User ID และวิธีผูกบัญชี; event กลุ่ม/ห้องและเนื้อหาข้อความจะไม่ถูกนำไปเก็บ ผู้พักนำ ID ไปผูกในหน้าโปรไฟล์และกรอกรหัสยืนยัน 6 หลักที่ส่งไปยัง LINE ปลายทาง โดยไม่ใช้ PIN ส่วนการส่งบิลใช้ `https://api.line.me/v2/bot/message/push`, retry ด้วย `X-Line-Retry-Key` เดิม และถือ HTTP 409 ว่าคำขอเดิมได้รับแล้ว
...
- LINE webhook ไม่ตอบ: ตรวจว่า `004_line_webhook.sql` รันแล้ว, ตั้งทั้ง Channel access token และ Channel secret, Webhook URL เป็น `<APP_URL>/api/webhooks/line`, เปิด Use webhook และ Webhook redelivery, กด Verify และตรวจว่า LINE เรียก HTTPS domain จริงได้ โดยห้ามพิมพ์ token/secret หรือลายเซ็นลง log
...
- หน้า Settings แจ้งว่า integration ยังไม่พร้อม: ฐานใหม่ให้ import `install.sql` (หรือ `schema.sql` + `defaults.sql` แบบขั้นสูง); ฐานเดิมต้องรัน `001_integration_settings.sql` เมื่อจำเป็น, `002_operational_hardening.sql` หนึ่งครั้ง, `003_append_only_guards.sql`, `004_line_webhook.sql`, `005_booking_active_phone.sql`, deploy transitional commit `a52bc33`, รัน `006_remove_resident_pin.sql`, ตรวจ schema แล้วจึง deploy source ปัจจุบัน จากนั้น login ด้วย role `owner`; ช่อง secret ว่างหมายถึงเก็บค่าเดิม ไม่ได้ล้างค่า
