...
- หน้า Settings แจ้งว่า integration ยังไม่พร้อม: ฐานใหม่ให้ import `install.sql` (หรือ `schema.sql` + `defaults.sql` แบบขั้นสูง); ฐานเดิมต้องรัน `001_integration_settings.sql` เมื่อจำเป็น, `002_operational_hardening.sql` หนึ่งครั้ง, `003_append_only_guards.sql`, `004_line_webhook.sql` และ `005_booking_active_phone.sql` จากนั้น login ด้วย role `owner`; ช่อง secret ว่างหมายถึงเก็บค่าเดิม ไม่ได้ล้างค่า
